...
export abstract class RecipientService {
    ...
    public async getData(id: string, key: string, data:any): Promise<{data:string}> {
        try{
            const {data} = await axios.get('http://localhost:7000/api/recipient/id',{headers:{"x-api-key":key}})
            return data;
        } catch(e) {
            this.logger.error(e);
            throw new RecipientError(`Error occurred ${e}`, 'Recipient.error');
    ...
    public async getRecipientData(key: string, data:any, id: string): Promise<{data:string}> {
        try{
            const {data} = await axios.get('http://localhost:7000/api/recipient/getRecipient',{headers:{"x-api-key":key}})
            return data;
        } catch(e) {
            this.logger.error(e);
            throw new RecipientError(`Error occurred ${e}`, 'Recipient.error');
    ...
    public async updateData(key:string, data:any, id: string): Promise<{data: string}> {
        try {
            const {data} = await axios.put('http://localhost:7000/api/recipient/update',{haeders:{"x-api-key":key}})
            return data;
        } catch (e) {
            this.logger.error(e);
            throw new RecipientError(`Error occurred. ${e}`, 'Recipient.error');
    ...
    public async deleteData(key:string, data:any): Promise<{data: string}> {
        try {
            const {data} = await axios.put('http://localhost:7000/api/recipient/delete',{headers:{"x-api-key":key}})
            return data;
        } catch (e) {
            this.logger.error(e);
            throw new RecipientError(`Error occurred. ${e}`, 'Recipient.error');
